...
import nyc.spookyrobotics.piserver.getHeader
...
abstract class ContextHandler {
    ...
    protected fun verifyRequestMethod(httpExchange: HttpExchange, method: String): Boolean {
        if (httpExchange.requestMethod != method) {
            sendUnauthorized(httpExchange)
            return false
        }
        return true
    }

    protected fun buildQueryMap(httpExchange: HttpExchange): Map<String, String> {
        val resultMap = mutableMapOf<String,String>()
        val queryString = httpExchange.requestURI.query
        queryString?.split("&")?.forEach { keyValuePair ->
            val parts = keyValuePair.split("=")
            if (parts.size != 2) {
                System.out.println("Invalid arguments parsed: $keyValuePair")
                return@forEach
            }
